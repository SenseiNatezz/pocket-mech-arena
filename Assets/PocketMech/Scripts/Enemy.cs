...
namespace PocketMech
{
    public sealed class Enemy : MonoBehaviour
    {
        ...
        public float Radius => Kind == EnemyKind.Boss ? 1.3f : Kind == EnemyKind.Elite ? .8f : .48f;
        float speed, attack, charge, chargeClock, contact;
        int bossPattern;
        ...
        public void Init(EnemyKind kind, float elapsed)
        {
            ...
            MaxHealth = kind == EnemyKind.Boss ? Game.Instance.balance.bossHealth : (kind == EnemyKind.Shield ? 560 : kind == EnemyKind.Rush ? 200 : kind == EnemyKind.Bomber ? 300 : 180) * scale;
            if (kind == EnemyKind.Elite) MaxHealth = 1800;
            if (Game.Instance.Profile.mission == 1) MaxHealth *= 1.3f;
            Health = MaxHealth; speed = kind == EnemyKind.Boss ? .8f : kind == EnemyKind.Shield ? 1.3f : kind == EnemyKind.Rush ? 2.3f : 1.7f;
        ...
        void Update()
        {
            ...
            if (Kind == EnemyKind.Bomber) movement *= distance > 6 ? 1 : .2f;
            if (Kind == EnemyKind.Boss) movement *= distance > 6 ? 1 : 0;
            ...
            MotionMath.Turn(turret != null ? turret : transform, dir, Kind == EnemyKind.Boss ? 100 : 300, dt);
            if (artwork != null) artwork.localPosition = new Vector3(0, Mathf.Sin(Time.time * 5 + phase) * (Kind == EnemyKind.Boss ? .015f : .045f), 0);
            contact -= dt;
            if (distance < Radius + .45f && contact <= 0) { contact = 1; g.Player.Hurt(Kind == EnemyKind.Boss ? 150 : 65); }
            attack -= dt;
            if (attack > 0) return;
            switch (Kind)
            {
        ...
        public void Hurt(float amount, Vector2 incoming, bool critical)
        {
            ...
            if (Kind != EnemyKind.Boss) knockback += incoming.normalized * .8f;
            ...
            if (Health <= 0)
            {
                Visuals.Burst(transform.position, Kind == EnemyKind.Boss ? Visuals.Red : new Color(1, .65f, .22f), Kind == EnemyKind.Boss ? 40 : 9);
