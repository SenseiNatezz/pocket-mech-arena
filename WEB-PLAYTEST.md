# Browser playtest — v0.5.0

Play: https://senseinatezz.github.io/pocket-mech-arena/

Open the link directly in Safari on iPhone (or a current desktop browser), tap Load Game, then Play → select Arena A-1 or Frostline → Start Mission → Confirm & Deploy. No installation or Apple Developer account is required for browser play. The first download is about 21 MB; leave the page open while it loads. This is a hosted Unity Web build, not an installed iOS app.
...
The underlying v0.5 Windows combat build passed 54 runtime regression checks. Those are separate from the browser checks and should not be read as 54 iPhone checks.

Public deployment verified: the GitHub Pages URL loads the actual Unity home screen, mission selection, loadout and combat with no browser errors observed. The repository About URL now points to the browser game.
